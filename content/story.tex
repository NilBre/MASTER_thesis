...
\begin{figure}
  ...
  \caption{Different ways of describing a measurement point inside the detector(quelle gibt es nicht mehr-> selber zeichnen).}
  \label{fig:dofs}
\end{figure}
...
The starting configuration used from Florian Reiss is called \textit{baseline}.
The degrees of freedom for the baseline are $Tx$ and $Rz$ with no constraints.
That way, a comparison to an unconstrained detector can be performed.
The track type used is \textit{GoodLongTracks} and the alignment was performed
for the stations and layers.
...
A clear evidence of this computing error can be seen in figure \ref{fig:june_3}. Here a comparison between a modified baseline configuration shown in the blue dots, and a new configuration using halflayer constraints shown with the orange dots, is performed.
Halflayers are, in contrast to full layers, the seperation into C-side and A-side of the layer.
With that constraints on each half can be implemented seperately.

This new configuration in orange converged after 15 iterations. Normally, an alignment job should converge after three to five iterations. If the convergence happens later there
could be a problem with the constraints which prevent the alignment from converging because they are implemented wrongly or weak modes hinder the convergence.
